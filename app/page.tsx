...
export default function Home() {
  ...
  // ✅ Fix: only render stars after client mounts
  ...
  return (
    <main className="h-screen w-full overflow-y-scroll scroll-smooth snap-y snap-mandatory scroll-pt-32">
      {/* Navbar */}
      <nav className="fixed top-6 w-[90%] max-w-6xl flex items-center justify-between px-8 py-4 bg-[#ffffff] backdrop-blur-lg rounded-2xl shadow-lg mx-auto left-1/2 -translate-x-1/2 z-50 font-poppins">
        ...
        <div className="text-[#EE5D00] font-extrabold text-lg tracking-widest">
          TWOCODED
        </div>
      ...
      {/* Hero Section with Gradient + Sparkling Stars */}
      <section className="h-screen w-full relative flex flex-col items-center justify-center text-center snap-start overflow-hidden">
        ...
        {/* ✅ Only render stars after mounted */}
        {mounted && (
      ...
      {/* Recent Works */}
      <section className="h-screen bg-white text-[#1b1c4f] relative flex flex-col justify-center items-center snap-start">
        <h2 className="absolute top-32 left-12 text-6xl font-extrabold uppercase z-20">
          Recent
        </h2>
        <h2 className="absolute top-32 right-12 text-6xl font-extrabold uppercase z-20">
          Works
        </h2>
        <div
        ...
        >
          <div className="relative w-[600px] h-[600px] flex justify-center items-center">
            {works.map((work, idx) => {
              ...
              const offset = (idx - activeIndex) * 520;
              return (
                <motion.div
                  key={`${work.id}-${idx}`}
                  animate={{
                ...
                >
                  <div
                    className="relative overflow-hidden shadow-2xl rounded-3xl transition-all duration-700 ease-in-out bg-white"
                    style={{
                      width: isActive ? "460px" : "340px",
                      height: isActive ? "600px" : "420px",
                    }}
                  >
                    ...
                    {isActive && (
                      <>
                        ...
                        <motion.div
                          ...
                          className="absolute bottom-8 left-6 text-left bg-white/90 px-6 py-4 rounded-2xl w-[85%]"
                        >
                          <h3 className="text-2xl font-bold">{work.title}</h3>
                          <p className="text-sm text-gray-700 mt-1">{work.subtitle}</p>
          ...
          <button
            onClick={handlePrev}
            className="absolute left-4 top-1/2 -translate-y-1/2 bg-[#000B41] text-[#EE5D00] p-3 rounded-full shadow-lg hover:bg-gray-800 transition"
          >
            &#10094;
          </button>
          <button
            onClick={handleNext}
            className="absolute right-4 top-1/2 -translate-y-1/2 bg-[#000B41] text-[#EE5D00] p-3 rounded-full shadow-lg hover:bg-gray-800 transition"
          >
            &#10095;
          </button>
        </div>
      ...
      <section className="h-screen w-full relative flex flex-col items-center justify-center text-center snap-start overflow-hidden">
        ...
        <h2 className="text-6xl md:text-7xl lg:text-8xl font-extrabold text-white relative z-20">
        ...
        <div className="relative w-72 h-72 md:w-96 md:h-96 my-16 z-20">
          <Image src="/b1.jpeg" alt="Central Visual" fill className="object-contain" />
        </div>
        <p className="text-white text-lg md:text-xl max-w-xl z-20">
          Start your journey with us today. We’re here to help bring your ideas to life.
        </p>
      </section>

      {/* Footer */}
      <footer className="bg-[#ffffff] text-[#000B41] py-16 px-12 relative snap-start h-f">
